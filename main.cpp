...
void stBuild_(int nd, int l, int r) {
    stl_[nd] = l;
    str_[nd] = r;
    if (l == r) {
        // TODO
    ...
    int mid = (l + r) >> 1;
    stBuild_(nd*2, l, mid);
    stBuild_(nd*2+1, mid+1, r);
    // TODO
}

void stRangeQuery(int nd, int l, int r) {
    if (stl_[nd] == l && str_[nd] == r) {
        // TODO
        return;
    }
    int mid = (stl_[nd] + str_[nd]) >> 1;
    if (r <= mid) stRangeQuery(nd*2, l, r);
    else if (l > mid) stRangeQuery(nd*2+1, l, r);
    else {
        stRangeQuery(nd*2, l, mid);
        stRangeQuery(nd*2+1, mid+1, r);
        // TODO
    }
}

void stPointUpdate(int nd, int pos) {
    if (stl_[nd] == str_[nd]) {
        // TODO
        return;
    }
    int mid = (stl_[nd] + str_[nd]) >> 1;
    if (pos <= mid) stPointUpdate(nd*2, pos);
    else stPointUpdate(nd*2+1, pos);
    // TODO
}

void stRangeUpdate(int nd, int l, int r) {
    if (stl_[nd] == l && str_[nd] == r) {
        // TODO
        return;
    }
    int mid = (stl_[nd] + str_[nd]) >> 1;
    if (r <= mid) stRangeUpdate(nd*2, l, r);
    else if (l > mid) stRangeUpdate(nd*2+1, l, r);
    else {
        stRangeUpdate(nd*2, l, mid);
        stRangeUpdate(nd*2+1, mid+1, r);
    }
    // TODO
}
